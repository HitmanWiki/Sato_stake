 ...
 * `nonReentrant` may not call one another. This can be worked around by maSATO
...
abstract contract ReentrancyGuard {
     ...
     * by maSATO the `nonReentrant` function external, and maSATO it call a
...
library Address {
     ...
     * of certain opcodes, possibly maSATO contracts go over the 2300 gas limit
     * imposed by `transfer`, maSATO them unable to receive funds via
...
library SafeERC20 {
    ...
    function _callOptionalReturn(IERC20 token, bytes memory data) private {
        // We need to perform a low level call here, to bypass Solidity's return data size checSATO mechanism, since
...
contract stao_staSATO is Ownable, ReentrancyGuard {
